...
public:
    ...
    std::map<KeyFrame*,size_t> GetObservations();  //只有关键帧才被算入observations中
    int Observations();
...
public:
    ...
    int nObs;  //该点被keyframe观测到的次数（单目+1,双目/rgbd +2）

    // Variables used by the tracking
    //下边这五个变量在Frame::isInFrustum被确定
    float mTrackProjX;
    float mTrackProjY;
    float mTrackProjXR;
...
protected:
    ...
    bool mbBad;
    MapPoint* mpReplaced;  //用于记录替换当前MapPoint(this)的MapPoint，用于Tracking::CheckReplacedInLastFrame()更新当前点
...
/*值得学习的地方
 * 1、描述子计算用能观测到该点的所有关键帧中的描述子来计算获得。计算每个i帧的描述子与其他帧的描述子之间的距离，然后获得距离的中位数，
 * 中位数最小的第i帧对应的描述子为该MapPoint的描述子
 * 2、用std::map<KeyFrame*,size_t> mObservations;这种数据结构来存储观测到this MapPoint的关键帧以及this MapPoint在该关键帧中对应的id。
 * 3、通过MapPoint与相机原点的距离，根据scale来预测观测到this MapPoint的所处的金字塔的层数。