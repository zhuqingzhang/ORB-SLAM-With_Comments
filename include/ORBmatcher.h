...
/*值得学习的地方
 * 1、统计kf和f匹配点对应的特征点的角度差，通过直方图统计，来过滤掉那些虽然匹配上了但是角度差与总体的角度差相差较大的匹配点  参见SearchByBoW
 * 2、在SearchByBoW中，kf和f都有成员mFeatVec，改变量中存储有特征点的Node信息（层级信息），通过NodeId的匹配来加速特征点匹配
 * 3、从sim矩阵中回复尺度因子： sim3=[sR, t]. s=sqrt(sR.row(0).dot(sR.row(0)). 因为sR为单位正交矩阵，所以第一行点积后为s^2
 *4、求极点： kf1的相机中心在kf2图片上的投影为 kf1在kf2上的极点坐标
 * 5、比如有 set<X*> s; 现在要知道 X* a这一指针是否在s中，可以用 s.count(a). 同理可用于map等