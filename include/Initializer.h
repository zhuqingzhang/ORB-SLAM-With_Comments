 ...
 *
 * 2、用两个线程同时计算H和F矩阵。另外注意在开线程的时候，调用类成员函数需要明确类对象(this),并且，如果成员函数的参数是引用类型的话，
 * 在thread传入参数的时候，需要使用ref()来传递参数。
 *3、求矩阵A的零空间h，或者当矩阵A不存在零空间时求Ah的最小值： 通过SVD求解Ah = 0，A'A最小特征值对应的特征向量即为解
 * 4、求解F矩阵时，先转化为Ah=0求解，得到的h reshape成3*3的矩阵Fpre。但是F矩阵的秩为2,需要进行约束。
 * 将Fpre进行SVD分解，并将奇异值w的最后一维设成0,然后再用U*diag(w)*Vt 恢复F矩阵。
 *
 *
 * /