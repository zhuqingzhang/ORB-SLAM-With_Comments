...
const int ORBmatcher::TH_HIGH = 100;    //用于特征点匹配时计算描述子间距离的阈值，距离小于TH_HIGH的时候，认为匹配成功
const int ORBmatcher::TH_LOW = 50;      //和TH_HIGH的作用一样，只不过要求更严科
const int ORBmatcher::HISTO_LENGTH = 30;

/**
 * Constructor
 * @param nnratio  ratio of the best and the second score，默认0.6，在Tracking::MonocularInitialization()初始化ORBmatcher时传入0.9
...
/**
 * @brief 对于每个局部3D点通过该店中的成员mTrackProjX，mTrackProjY，搜索F中以mTrackProjX，mTrackProjY为圆心，r为半径的所有2D点
 * 找到和最匹配的2D点。从而实现Frame对Local MapPoint的跟踪。用于tracking过程中实现当前帧对局部3D点的跟踪。
 *
...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        // step3：在2D投影点附近一定范围内搜索属于miniLevel~maxLevel层的特征点 ---> vIndices 特征点的id
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            if(F.mvuRight[idx]>0)
            {
                const float er = fabs(pMP->mTrackProjXR-F.mvuRight[idx]);  //如果pMP->mTrackProjXR和F.mvuRight[idx]相差过大，说明pMP与F中id为idx的特征点不匹配
        ...
        if(bestDist<=TH_HIGH)
        {
            if(bestLevel==bestLevel2 && bestDist>mfNNratio*bestDist2)  //如果best and second在同一scale level，就得保证bestdist要小于等于0.6*bestdist2
                continue;
...
bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint &kp1,const cv::KeyPoint &kp2,const cv::Mat &F12,const KeyFrame* pKF2)
{
    ...
    return dsqr<3.84*pKF2->mvLevelSigma2[kp2.octave];   //kp2到极线距离的平方应该小于该层的不确定度才可以。
}
 ...
 * @param  vpMapPointMatches 存储与pKF中MapPoints相匹配上的F中MapPoints，NULL表示未匹配， vector的id表示F中keypoint的id， MapPoints*为pKF中的点
 * @return                   成功匹配的数量
 */
int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
{
    const vector<MapPoint*> vpMapPointsKF = pKF->GetMapPointMatches(); //直接返回pKf中的mvpMapPoints，里边可能包含空指针和坏点

    vpMapPointMatches = vector<MapPoint*>(F.N,static_cast<MapPoint*>(NULL));

    const DBoW2::FeatureVector &vFeatVecKF = pKF->mFeatVec;  //基类是std::map<NodeId, std::vector<unsigned int> >
                                                            //每一个节点Node对应一个vector<int>,存储的是local feature 的id

    int nmatches=0;

    vector<int> rotHist[HISTO_LENGTH];   //统计直方图分为30段，每段代表360/30 度
    ...
    while(KFit != KFend && Fit != Fend)
    {
        if(KFit->first == Fit->first) //步骤1：分别取出属于同一node的ORB特征点(只有属于同一node，才有可能是匹配点)
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                for(size_t iF=0; iF<vIndicesF.size(); iF++)
                {
                    ...
                    if(vpMapPointMatches[realIdxF])// 表明F中的这个点已经被匹配过了，不再匹配，加快速度
                        continue;
        ...
        else if(KFit->first < Fit->first)   //如果KFit的节点ID 小于Fit的节点ID
        {
            KFit = vFeatVecKF.lower_bound(Fit->first);   //对vFeatVecKF进行搜索，找到节点ID大于等于Fit的节点ID对应的指针
        }
        else
        {
    ...
    if(mbCheckOrientation)
    {
        ...
        // 计算rotHist中最大的三个的index，如果最大的三个中的某一个或者某两个和另外一个的存储数量不再一个量级，则将其置为-1
...
// 用于闭环检测中将vpPoints中的MapPoint和关键帧pKF的特征点进行关联，并将匹配上的点存入vpMatched中
// 根据Sim3变换，将每个vpPoints投影到pKF上，并根据尺度确定一个搜索区域，
// 根据该MapPoint的描述子与该区域内的特征点进行匹配，如果匹配误差小于TH_LOW即匹配成功，更新vpMatched
//与第一个int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)不同的是
//在此处我们直接将vpPoints中的MapPoint根据Scw向pKF中投影，得到投影点u，v，  该uv与上边这个函数的mTrackProjX，mTrackProjY相对应。
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    const float scw = sqrt(sRcw.row(0).dot(sRcw.row(0)));   // 计算得到尺度s. 因为sR为单位正交矩阵，所以第一行点积后为s^2
    cv::Mat Rcw = sRcw/scw;
    ...
    for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
    {
        ...
        if(PO.dot(Pn)<0.5*dist) //PO和Pn的夹角大于60度
            continue;
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        if(level1>0)
            continue;
        //认为可能与vbPrevMatched[i1].x,vbPrevMatched[i1].y匹配的点在第二帧中位置相差不会太大，
        // 在以x.y为中心的windowsize大小的框中中找到符合level1层的所有F2的特征点id，放于vIndces2中，用于后续筛选
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            {
                if(vnMatches21[bestIdx2]>=0)  //如果F2中第bestIdx2的特征点已经与F1中的特征点有匹配了，则删除,更换为新的更好的匹配点
                {
                    vnMatches12[vnMatches21[bestIdx2]]=-1;
                    nmatches--;
                }
                vnMatches12[i1]=bestIdx2;      //F1中的特征点i1应该对应于F2中的特征点bestIdx2
                vnMatches21[bestIdx2]=i1;    //F2中的特征点bestIdx2对英语F1中的特征点i1
 ...
 * @param  vpMatches12        pKF2中与pKF1匹配的MapPoint，null表示没有匹配。vector的id表示pKF1中keypoint的id，MapPoint*为pKF2中的点。
 * @return                    成功匹配的数量
 */
int ORBmatcher::SearchByBoW(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches12)
{
    // 详细注释可参见：SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
    // vpMapPointMatches与vpMatches12的含义略有不同
        ...
        if(f1it->first == f2it->first)//步骤1：分别取出属于同一node的ORB特征点(只有属于同一node，才有可能是匹配点)
        {
            ...
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    if(vbMatched2[idx2] || !pMP2)    //pKF2中的idx2这个点已经有了匹配点了
                        continue;
 ...
 * ！！！！这里与上边的那些函数不同。上边的函数都是匹配已经有3D点的特征点，而此处是匹配没有对应3D点的特征点。
 * @param pKF1          关键帧1
 * @param pKF2          关键帧2
 * @param F12           基础矩阵
 * @param vMatchedPairs 存储匹配特征点对，特征点用其在关键帧中的索引表示， pair中的第一个参数为pKF1中点的索引，第二个参数是pKF2中点的索引
        ...
        if(f1it->first == f2it->first)
        {
            ...
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    // 那么这个索引idx2就不能被考虑
                    if(vbMatched2[idx2] || pMP2)   //注意此处是pMP2,不是!pMP2  这里是寻找那些MapPoint为NULL的keypoint，匹配后将这些keypoint三角化，获得MapPoint
                        continue;
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for(int i=0; i<nMPs; i++)
    {
        ...
        if(pMP->isBad() || pMP->IsInKeyFrame(pKF))  //pMP已经被该关键帧pKF观测到了，即pMP->mObervations()中已经有pKF了
            continue;
...
// 与上边的fuse不同的是，上边的fuse直接在函数里边进行替换，而此处的fuse则是将需要替换的MapPoint记录在vpReplacePoint中
//vpReplacePoint的_ID_为vpPoints中的id， vpReplacePoint的值为需要被替换的MapPoint。 即用vpPoints[_ID_]来替换vpRepalcePoint[_ID_]
int ORBmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
{
    ...
    for(int iMP=0; iMP<nPoints; iMP++)
    {
        ...
        // Discard Bad MapPoints and already found
        if(pMP->isBad() || spAlreadyFound.count(pMP))  //TODO::为啥不用 pMP->IsInKeyFrame(pKF)?
            continue;
                             ...
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        // Match to the most similar keypoint in the radius
        const cv::Mat dMP = pMP->GetDescriptor();  //TODO:为啥不用pKF1->mDescriptors.row(i1)
...
int ORBmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
{
    ...
    {
        unsigned  int v = *pa ^ *pb;  //异或，相同为0，不同为1
