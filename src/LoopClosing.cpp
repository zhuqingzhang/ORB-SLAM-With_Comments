...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckFinish())  //见车有没有RequestFinish的请求
            break;
...
bool LoopClosing::DetectLoop()
{
    ...
    // 步骤1：如果距离上次闭环没多久（小于10帧），或者map中关键帧总共还没有10帧，则不进行闭环检测，直接将当前KF加入到KFDB中
    ...
    // 1、每个候选帧将与自己相连的关键帧构成一个“子候选组spCandidateGroup”，对于n个闭环候选帧有n个子候选组spCandidateGroup。
    // 2、遍历这n个子候选组：对于每一个子候选组spCandidateGroup，检测“子候选组”中是否有KF存在于之前的子候选组集合mvConsistentGroups中。
    // mvConsistentGroups存放了很多组之前的子候选组，并且每一个子候选组对应了一个数字，该数字表示有几个子候选组与该子候选组相关联
    // 只要当前的子候选组spCandidateGroup中有一个KF与mvConsistentGroups中的某个子候选组mvConsistentGroups[iG]中的KF是同一个KF，
    //则当前子候选组spCandidateGroup就找到了与之相关联的之前的子候选组mvConsistentGroups[iG]，nCurrentConsistency=mvConsistentGroups[iG].second+1.
    //之前的子候选组mvConsistentGroups[iG]已经有mvConsistentGroups[iG].second个子候选组与之相关联了，当当前子候选组spCandidateGroup又与
    //mvConsistentGroups[iG]相关联后，说明当前的子候选组spCandidateGroup有（mvConsistentGroups[iG].second+1）个子候选组与之相关联，
    // 如果当前的spCandidateGroup之前未被处理过，将当前的spCandidateGroup和与之相关联的子候选组的组数makepair，放入vCurrentConsistentGroups，
    //在整个循环结束后用于更新 mvConsistentGroup
    ...
    // ConsistentGroup.first对应每个“子候选组”中的关键帧，ConsistentGroup.second为与该子候选组相关联的子候选组的组数
 ...
 * 2. 根据估计的Sim3，对3D点进行投影找到更多匹配，通过优化的方法计算更精确的Sim3，并记录内点数，选择第一个内点数大于20的帧作为闭环帧（当前帧---闭环帧）
 * 3. 将闭环帧以及闭环帧相连的关键帧的MapPoints与当前帧的点通过matcher.SearchByProjection进行匹配，找到当前帧更多的匹配点。匹配点数大于40则成功（当前帧---闭环帧+相连关键帧）
 * 
...
bool LoopClosing::ComputeSim3()
{
    ...
    for(int i=0; i<nInitialCandidates; i++)
    {
        ...
        // 参与Sim3计算的候选关键帧数加1
        nCandidates++;
    }//遍历闭环候选帧，对于每一个闭环候选帧，与当前关键帧进行BOW匹配，匹配点数不少于20对时，构建Sim3Solver求解器，候选关键帧计数加一
...
void LoopClosing::CorrectLoop()
{
    ...
    CorrectedSim3[mpCurrentKF]=mg2oScw;  //当前帧Sim3位姿
    cv::Mat Twc = mpCurrentKF->GetPoseInverse();  //当前帧SE3位姿
...

    {
        ...
        // 步骤2.1：得到与CurKF相连的KF经过Sim3调整后的位姿（只是得到，还没有修正）
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            // 与CurKF相连的KF没有进行mg2oScw修正的位姿
            NonCorrectedSim3[pKFi]=g2oSiw;
        }

        // Correct all MapPoints obsrved by current keyframe and neighbors, so that they align with the other side of the loop
        // 步骤2.2：步骤2.1得到与CurKF相连的KF修正过的位姿后，将这些KF的MapPoint的坐标也进行相应的修正
    ...
    // Fuse duplications.
    // 步骤4：利用CorrectedSim3中的KF和Pose，将mvpLoopMapPoints投影到这些KF中，进行MapPoints检查与替换
    ...
    // 步骤5.1：遍历当前帧相连的关键帧，包括当前帧（一级相连）
    for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
    {
        KeyFrame* pKFi = *vit;
        // 步骤5.2：得到与当前帧相连关键帧的相连关键帧（二级相连）
        vector<KeyFrame*> vpPreviousNeighbors = pKFi->GetVectorCovisibleKeyFrames();  //pKFi原本相关联的KF

        // Update connections. Detect new links.
        // 步骤5.3：更新一级相连关键帧的连接关系
        pKFi->UpdateConnections();
        // 步骤5.4：取出该帧更新后的连接关系
        LoopConnections[pKFi]=pKFi->GetConnectedKeyFrames();  //pKFi进行了UpdateConnections之后相关联的KF
    ...
    mpLocalMapper->Release();    //清除NewKeyFrames
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
            ...
            while(!lpKFtoCheck.empty())
            {
                ...
                pKF->mTcwBefGBA = pKF->GetPose(); //GBA优化之前的位姿
