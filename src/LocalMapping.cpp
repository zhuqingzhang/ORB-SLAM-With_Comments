...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        SetAcceptKeyFrames(false);   //在Tracking：：NeedNewKeyFrame中调用
        ...
        if(CheckNewKeyFrames())
        {
            ...
            //从候选的关键帧mlNewKeyFrame中选出最前边的一帧设为CurrentKeyFrame
            // 计算关键帧特征点的BoW映射，获得当前KF的MapPoints,如果MapPoint是好点，且之前没被当前KF 观测到，
            // 则给这个MapPoint添加当前帧的观测，同时更新MapPoint的深度、方向、描述子。 否则将这个MapPoint加入 mlpRecentAddedMapPoints等待处理。
            // 更新关键帧的共视图和Enssential graph， 将关键帧插入地图
            ProcessNewKeyFrame();

            // Check recent MapPoints
            // VI-B recent map points culling
            //对mlpRecentAddedMapPoints进行处理，剔除ProcessNewKeyFrame函数中引入的不合格MapPoints
            MapPointCulling();

            // Triangulate new MapPoints
            // VI-C new map points creation
            //找到与CurrentKeyFrame共识最好的10帧关键帧，然后依次与这些关键帧进行特征点匹配，将匹配到的点
            // 通过三角化再新恢复出一些MapPoints点，加入到地图中，同时将MapPoint和对应嗯的关键帧相关联
            CreateNewMapPoints();
            ...
            if(!CheckNewKeyFrames())
            {
                ...
                // 检查并融合当前关键帧与相邻帧（两级相邻）重复的MapPoints,然后更新融合后的点的描述子等特性，
                //更新当前关键帧的共视图
                SearchInNeighbors();
            }
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpMap->KeyFramesInMap()>2)
                   ...
                   //在LocalBundleAdjustment中已经将优化后重投影误差过大的MapPoint和与之对应的KF的相互关联剔除了
        ...
        //每处理完一次，检查一下有没有Reset的要求
        ResetIfRequested();

        // Tracking will see that Local Mapping is not busy
        SetAcceptKeyFrames(true);  //只要处理完一帧关键帧，就可以接收新的关键帧。
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    {
        if(i>0 && CheckNewKeyFrames())  //如果已经处理了一帧相邻关键帧，且队列NewKeyFrames中还有新的关键帧，则退出循环
            return;
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            // 由相机坐标系转到世界坐标系，得到视差角余弦值(两个相机对该点观测视线的夹角
            ...
            /*if(fabs(ratioDist-ratioOctave)>ratioFactor)
                continue;*/
            //ratioOctave= 1.5f*mpCurrentKeyFrame->mfScaleFactor=1.5*1.2;
            //(dist2*scale2)/(dist1*scale1)<1/(1.5*ScaleFactor)  || (dist2*scale2)/(dist1*scale1)> (1.5*ScaleFactor)/1
            //也就是说，这两个距离的在金字塔尺度下的比值不能超过大约两个尺度值(1.5*1.2),否则就认为尺度不连续
...
void LocalMapping::KeyFrameCulling()
{
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        if(nRedundantObservations>0.9*nMPs)
            pKF->SetBadFlag();  //剔除之前插入的被认为是冗余的关键帧
    }
}
