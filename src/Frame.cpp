     ...
     mpReferenceKF(static_cast<KeyFrame*>(NULL))
{
    ...
    // ORB extraction
    // 同时对左右目提特征
    thread threadLeft(&Frame::ExtractORB,this,0,imLeft);  //0代表左目
    thread threadRight(&Frame::ExtractORB,this,1,imRight);
    threadLeft.join();
    threadRight.join();
    //此时mvKeys,mDescriptors,mvKeysRight,mDescriptorsRight已经有了相应的数据
    N = mvKeys.size();
...
{
    if(flag==0)
        (*mpORBextractorLeft)(im,cv::Mat(),mvKeys,mDescriptors);  //对应与ORBExtractor的（）重载运算，第二个参数是Mask
    else
 ...
 * @param  pMP             MapPoint
 * @param  viewingCosLimit 视角和平均视角的方向阈值，设为cos60
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    // 世界坐标系下，相机到3D点P的向量, 向量方向由相机指向3D点P,注意PO是在世界坐标系下的。
...
void Frame::UndistortKeyPoints()
{
    ...
    for(int i=0; i<N; i++)
    {
        cv::KeyPoint kp = mvKeys[i];  //得到keypoint
        kp.pt.x=mat.at<float>(i,0);   //将该keypoint中的坐标改为去畸变后的坐标，其余的成员变量(角度、层数等）保持不变
...
void Frame::ComputeStereoMatches()
{
    ...
    for(int iR=0; iR<Nr; iR++)
    {
        ...
        const float r = 2.0f*mvScaleFactors[mvKeysRight[iR].octave];  //因为此时kpY是最底层图像下的坐标，要让搜索带在每一层的宽度一样，则需要考虑scalefactor
    ...
    const float minZ = mb;        // NOTE bug mb在构造函数中被初始化为0，mb的正确赋值在构造函数中放在ComputeStereoMatches函数的后面
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        // 可能的匹配点
        //vRowIndeces[vL]中已经存放了在右目图像中的特征点的id。
        //因为在上一步中，右目图像中的特征点纵坐标被扩大成了带状的，所以同一行中有很多不同的特征点id。
        ...
        for(size_t iC=0; iC<vCandidates.size(); iC++)
        {
            ...
            if(uR>=minU && uR<=maxU)  //实际上此处的minU是负无穷
            {
        ...
        if(bestDist<thOrbDist)
        {
            ...
            const float scaleduR0 = round(uR0*scaleFactor); //虽然mvKeysRight[bestIdxR]被提取的金字塔层可能和kpL不同，但是我们将其坐标scale到与kpL相同的层
...
cv::Mat Frame::UnprojectStereo(const int &i)
{
    ...
    // 事实上，由于双目图像本来就是只有去畸变后才能在该系统中进行双目匹配，所以对于双目来说进不进行去畸变的操作都是一样的。去畸变操作是冗余操作
