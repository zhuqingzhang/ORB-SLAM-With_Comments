    ...
    mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale)
{
    ...
    for(int i1=0; i1<mN1; i1++)
    {
        ...
        if(vpMatched12[i1])
        {
            ...
            //9.21 为自由度为2是随即变量卡方的值小于9.21的概率为0.99。
            // 即 t^2=F_2(0.99)^-1*sigmaSquare=9.210*sigmaSquare, error^2<t^2 为内点，概率为0.99
    ...
    //默认参数为probability = 0.99, minInliers = 6 , maxIterations = 300
    SetRansacParameters();
}
...
void Sim3Solver::SetRansacParameters(double probability, int minInliers, int maxIterations)
{
    ...
    mRansacMinInliers = minInliers;  //6
    ...
    if(mRansacMinInliers==N)  //如果内点数N和所需要的最少采样点数一样，只用迭代一次就行了，也没多余的点可以选择
        nIterations=1;
    else
        nIterations = ceil(log(1-mRansacProb)/log(1-pow(epsilon,3)));   //一次取3对点来计算sim3

    mRansacMaxIts = max(1,min(nIterations,mRansacMaxIts)); //迭代次数不要超过300次
...
cv::Mat Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)
{
    ...
    while(mnIterations<mRansacMaxIts && nCurrentIterations<nIterations)
    {
        ...
        if(mnInliersi>=mnBestInliers)  //mnBestInliers初始值为0
        {
            mvbBestInliers = mvbInliersi; //记录N个MapPoints中哪些是内点，相应的mvbInliersi[i]=true
            mnBestInliers = mnInliersi;  //内点数
...
void Sim3Solver::ComputeSim3(cv::Mat &P1, cv::Mat &P2)
{
    ...
    // Horn 1987, Closed-form solution of absolute orientataion using unit quaternions  （此处用寺院书表示了旋转矩阵）

    // Horn 1988 Closed-Form Solution of Absolute Orientation using Orthonormal Matrices (此处用正交矩阵表示了旋转矩阵）
    ...
    //如果M=Pr2*Pr1.t()则计算出来的是R12  如果M=Pr1*Pr2.t() 则计算出来的是R21
    cv::Mat M = Pr2*Pr1.t();
    ...
    cv::eigen(N,eval,evec); //evec.row(0) is the quaternion of the desired rotation
