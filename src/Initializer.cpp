 ...
 * @param sigma          测量误差  默认1
 * @param iterations     RANSAC迭代次数 默认200
 */
                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    // 步骤1：组织特征点对 {i, matched(i)}  i为refframe的特征点id， matched[i]为curframe的特征点id
    ...
    // 用于FindHomography(最少需要4对点)和FindFundamental(最少需要8对点)求解。
    // mMaxIterations:200
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        {
            // 产生0到N-1的随机数
            int randi = DUtils::Random::RandomInt(0,vAvailableIndices.size()-1);  //每次循环，size是依次减一的，因为vAvailableIndices.pop_back();
            // idx表示哪一个索引对应的特征点被选中
    ...
    // ref是引用的功能:http://en.cppreference.com/w/cpp/utility/functional/ref，保证传入该线程的参数和此处的参数地址相同
    //this表明是当前对象成员函数
    // 计算homograpy并打分
    ...
    // 步骤4：计算得分比例，选取某个模型
    float RH = SH/(SH+SF);   //由于评分准则的原因，SH的得分通常要小于SF的得分，所以RH的阈值要比0.5小一些
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // 恢复原始的均值和尺度
        //设原始尺度的两点为p1,p2,归一化后的两点为p10=T1*p1,p20=T2*p2
        // p20=Hn*p10--> T2*p2=Hn*T1*p1--> p2=(T2inv*Hn*T1)*p1
        H21i = T2inv*Hn*T1;
        H12i = H21i.inv();
...
{
    // Number of putative matches
    const int N = vbMatchesInliers.size();  //TODO：： vbMatchesInliers之前没有分配空间，应该换成mvMatches12.size();
                                            //在CheckFundamental的时候重新用mvMatches12.size()对N赋了值，所以不影响
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        //p20^t*Fn*p10=0--> (T2*p2)^t*Fn*(T1p1)=0-->F=T2^t*Fn*T1;
        F21i = T2t*Fn*T1;
...
cv::Mat Initializer::ComputeH21(const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2)
{
    ...
    return vt.row(8).reshape(0, 3); // vt的最后一行  reshape(0,3) 0表示通道数不变，3表示重构成3行的矩阵
}
...
{   
    ...
    //重投影误差的平方应该有95%的概率小于5.991*simga^2，这里sigma是1
    const float th = 5.991;  //Multiple View Geometry in Computer Vision   model selection 4.7.1 RANSAC p89
    ...
    for(int i=0; i<N; i++)
    {
        ...
        // x2in1 = H12*x2
        // 将图像2中的特征点单应到图像1中, a为尺度因子
        // |u1|    |h11inv h12inv h13inv||u2|
        // |v1| = a|h21inv h22inv h23inv||v2|
        ...
        const float chiSquare2 = squareDist2*invSigmaSquare;  //该数值越小越好

        if(chiSquare2>th)
            bIn = false;
        else
            score += th - chiSquare2;            //该数值越大越好
...
float Initializer::CheckFundamental(const cv::Mat &F21, vector<bool> &vbMatchesInliers, float sigma)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(chiSquare1>th)
            ...
            score += thScore - chiSquare1;  //因为单映矩阵的score就是以thScore为基础的，所以此处的基本矩阵评分也以此为基础
                                            //不过这样一来，基本矩阵的评分就比单应矩阵高，在最后筛选时需要进行衡量
                      ...
                      cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    //p2=K(R-t*n^T/d)K^-1*p1  --> p2=H21*p1  这里的p1和p2是像素坐标， 我们要分解的是 A=(R-t*n^T/d)=K^-1*H21*K
    cv::Mat invK = K.inv();
    cv::Mat A = invK*H21*K;
    ...
    vector<cv::Point3f> bestP3D; //存放着在第一帧相机坐标系下的3d点
    vector<bool> bestTriangulated;  //对应的id为mvMatches12的id，true表示成功三角化，false表示失败
                       ...
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        // Check parallax
        // 步骤4：计算视差角余弦值
        cv::Mat normal1 = p3dC1 - O1;   //相对第一帧坐标系，这里当成世界坐标系
        ...
        // 步骤6.1：重投影误差太大，跳过淘汰
        // 一般视差角比较小时重投影误差比较大
        if(squareError1>th2)  //th2为 （2*sigma）的平方， sigma默认为一个像素
            continue;
        ...
        if(cosParallax<0.99998)  //视差角不能太小
