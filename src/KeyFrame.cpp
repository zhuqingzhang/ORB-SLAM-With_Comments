    ...
    void KeyFrame::UpdateBestCovisibles()
{
    ...
    // 按照权重进行排序
    sort(vPairs.begin(),vPairs.end());  //按照weight从小到大排序
    //也可以直接进行从大到小排序：  sort(vPairs.begin(),vPairs.end(),[](pair<int,Keyframe*> i, pair<int,Keyframe*> j){return i.first>j.first ;});
    ...
    {
        lKFs.push_front(vPairs[i].second);   //push_front使得得到的list是按照weight从大到小排的
...
vector<KeyFrame*> KeyFrame::GetCovisiblesByWeight(const int &w)
{
    ...
    // 从mvOrderedWeights找出第一个小于w的那个迭代器
    //upper_bound 和lower_bound 必须用于已经按<或comp规则排好序的序列上，同时upper_bound和lower_bound的比较规则应该与之前排序序列的<或comp相同。
    //upper_bound和lower_bound的默认比较规则的 return i<j ; 分别返回第一个大于w 和 大于等于w 的地址。
    //由于此处 mvOrderedWeights是从大到小排列的，所以应该用比较规则： weightComp(int i, int j){return i>j;}，这样才能得到正确的结果。
    //此时，在weightComp的比较规则下，upper_bound返回的是第一个小于w的地址，lower_bound返回的是第一个小于等于w的地址。
    vector<int>::iterator it = upper_bound(mvOrderedWeights.begin(),mvOrderedWeights.end(),w,KeyFrame::weightComp);
    //如果it==mvOrderedWeights.end()有两种情况：
    // 1、说明mvOrderedWeights中的所有元素都大于等于w--->else。
    // 2、当*mvOrderedWeights.rbegin()<w ，说明w大于mvOrderedWeights中的所有元素，此时不存在大于等于w的关键帧，应该返回空vector --->if
    if(it==mvOrderedWeights.end() && *mvOrderedWeights.rbegin()<w)
        return vector<KeyFrame*>();
    else
    {
        int n = it-mvOrderedWeights.begin();
        return vector<KeyFrame*>(mvpOrderedConnectedKeyFrames.begin(), mvpOrderedConnectedKeyFrames.begin()+n); //weight大于等于w的值
    }
}
...
{
    int idx = pMP->GetIndexInKeyFrame(this); //返回this Keyframe中pMP的id值
    if(idx>=0)
        mvpMapPoints[idx]=static_cast<MapPoint*>(NULL);
}
...
set<MapPoint*> KeyFrame::GetMapPoints()
{
    ...
    {
        if(!mvpMapPoints[i]) //mvpMapPoints[i]为空，跳过
            continue;
...
/**
 * @brief 该关键帧中的MapPoint被所有帧观测到的次数大于等于minObs的那些MapPoints的数量
...
int KeyFrame::TrackedMapPoints(const int &minObs)
{
    ...
    for(int i=0; i<N; i++)   //N 是keypoint的个数
    {
        MapPoint* pMP = mvpMapPoints[i];
        if(pMP)   //如果id为i的keypoint分配了MapPoint
        {
            if(!pMP->isBad())
            {
                if(bCheckObs)
                {
                    ...
                    if(mvpMapPoints[i]->Observations()>=minObs)  //mvpMapPoints[i]被观测到的次数大于等于minObs
                        nPoints++;
                }
                else
                    nPoints++;     //如果minObs小于0,则不做判断，只要MapPoint 有值且不是坏点，就算。
            }
        }
    }
...
{
    // 在没有执行这个函数前，关键帧只和MapPoints之间有连接关系，这个函数可以更新关键帧之间的连接关系(即mvpOrderedConnectedKeyFrames）
    ...
    for(vector<MapPoint*>::iterator vit=vpMP.begin(), vend=vpMP.end(); vit!=vend; vit++)
    {
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(mit->first->mnId==mnId)
                continue;
            KFcounter[mit->first]++;  // keyframe（mit->first)与this keyframe的共视点数量+1.
        }
    }
    ...
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
        ...
        if(mit->second>=th)
        {
            ...
            (mit->first)->AddConnection(this,mit->second);  //是给相关联的keyframe加入和this keyframe之间的连接
    ...
    //===============3==================================
    {
        ...
        // 更新图的连接(权重)
        //KFcounter中是没有经过权重筛选的，只要有keyframe和this有共视点，哪怕只有一个，也在KFcounter中
        mConnectedKeyFrameWeights = KFcounter;//更新该KeyFrame的mConnectedKeyFrameWeights，更新当前帧与其它关键帧的连接权重
        //lKFs和lWs是从vPairs中得到的，vPairs是按照权重阈值th筛选得到的，其中的keyframe和this的共视点比较多
        mvpOrderedConnectedKeyFrames = vector<KeyFrame*>(lKFs.begin(),lKFs.end());
        mvOrderedWeights = vector<int>(lWs.begin(), lWs.end());

        // 更新生成树的连接
        if(mbFirstConnection && mnId!=0)  //如果是首次进行关键帧关联的操作，且不是第一帧关键帧，需要更新生成树
        {
            // 初始化该关键帧的父关键帧为共视程度最高的那个关键帧
            mpParent = mvpOrderedConnectedKeyFrames.front();
            // 建立双向连接关系，即父关键帧的一个子关键帧就是this keyframe
...
void KeyFrame::SetErase()
{
    ...
    if(mbToBeErased)  //应该是(!mbToBeErased)  mbToBeErased被初始化为false,如果是(mbToBeErased)，永远无法执行SetBadFlag()
    {
        SetBadFlag();
    }
}

void KeyFrame::SetBadFlag() //剔除冗余关键帧的时候调用 LocalMapping::KeyFrameCulling()
{   
    {
        unique_lock<mutex> lock(mMutexConnections);
        if(mnId==0)
...

    {
        ...
        while(!mspChildrens.empty())
        {
            ...
            } //遍历this Keyframe 所有的子节点mspChildrens，在这些子节点中找到一个子节点pC的相连关键帧pP在候选父节点中，且该子节点pC与pP的权重是最大的
              //疑问：为什么不在这个大的for循环中一次性地找到每一个子节点的父节点，而是只找一个子节点的父节点？
              //答：可能是因为这样的话，只能按mspChildrens中的顺序依次查找每一个子节点的父节点。如果第1个子节点找到了一个父节点，而第3个子节点找到父节点后
              //也会加入到sParentCandidates中，并且说不定第3个子节点作为第1个子节点的父节点更加合适，而此时就无法修改了。
              //其实也可以修改一下执行的逻辑.....
...
float KeyFrame::ComputeSceneMedianDepth(const int q)
{
    ...
    for(int i=0; i<N; i++)
    {
        if(mvpMapPoints[i])
        {
            ...
            float z = Rcw2.dot(x3Dw)+zcw; // (R*x3Dw+t)的第三行，即在相机坐标系下的z值
            vDepths.push_back(z);
        }
    }
    ...
    return vDepths[(vDepths.size()-1)/q];  //当q=2时，得到的是场景（this keyframe观测到的特征点）深度（在相机系下）的中位数
}
