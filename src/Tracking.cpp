    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    {
        // 判断一个3D点远/近的阈值 mbf * 35 / fx = baseline(m) * "ThDpeth"  对于一个基线为10cm的双目，大约等于3.5m。超过了3.5m就算远距离
    ...
    {
        // 深度相机disparity转化为depth时的因子,tum-rgbd 为5000
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    if((fabs(mDepthMapFactor-1.0f)>1e-5) || imDepth.type()!=CV_32F)
        imDepth.convertTo(imDepth,CV_32F,mDepthMapFactor);  //给深度图除以5000,这样深度图的数值就是实际的深度(m)
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)// 没有成功初始化的前一个状态就是NO_IMAGES_YET
        mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth); //没有初始化的mpIniORBextractor
    else                                                                                                      //要比初始化后的mpORBextractorLeft
        mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);//多提取一倍的特征点
    ...
    else// 步骤2：跟踪
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                // Fuse和SearchAndFuse只会更新Keyframe中的MapPoint，在这里我们也需要对last frame这种普通帧的MapPoint更新
            ...
            {
                // 找到候选relocalized的KFs，BOW搜索配对点，对每一个候选KF，EPnP求解位姿，位姿优化，直到有一个KF对应的内点足够多，
                //则重定位成功。如果对于某一个KF， EPnP Ransac求解次数达到上限，则该KF抛弃。如果最后所有的候选KF都不满足要求，
                //则重定位失败
                bOK = Relocalization();
            }
        }
        else
        {
            ...
            // 只进行跟踪tracking，局部地图不工作,mbOnlyTracking=true
            ...
            else
            {
                ...
                // mbVO为true表明此帧匹配了很少的MapPoints，少于10个，要跪的节奏(在Tracking::TrackWithMotionModel末尾有设置)
                if(!mbVO)
                {
                ...
                else
                {
                    ...
                    if(!mVelocity.empty())
                    {
                        ...
                        // 先将CurrentFrame跟踪上的MapPoints，Outlier,Tcw保存在 vpMPsMM vbOutMM TcwMM
                        //因为Relocalization()会修改MapPoints Outlier Tcw
                    ...
                    if(bOKMM && !bOKReloc)
                    {
                        //给CurrentFrame的相关变量赋值，这些变量是在TrackWithMotionModel下计算出来的
        ...
        // 步骤2.2：在帧间匹配得到初始的姿态后，现在对local map进行跟踪，TrackLocalMap,得到更多的匹配，并优化当前位姿
        ...
        {
            if(bOK)
                bOK = TrackLocalMap(); //更新LocalMap，从LocalMap中得到更多当前帧跟踪匹配上的点
        }
        else
        {
        ...
        if(bOK)
        {
            ...
            for(int i=0; i<mCurrentFrame.N; i++)
            {
                ...
                if(pMP)
                    // 在UpdateLastFrame函数中那些为了跟踪而增加的MapPoints并没有进行AddObservation的操作
                    //所以这些点的pMP->Observations()<1
                    if(pMP->Observations()<1)
                    {
                        //将临时添加的这些MapPoint从当前帧删除
            ...
            //但是这些点并没有在CeateNewKeyFrame中删除，我们允许这些点传递到New KeyFrame中，
            //但是不想让这些点传递到LastFrame中作为下一个普通帧追踪的MapPoint
            for(int i=0; i<mCurrentFrame.N;i++)
            {
                if(mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
                    mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        // 步骤4：在局部地图中添加该初始关键帧，等待处理
        mpLocalMapper->InsertKeyFrame(pKFini);

        mLastFrame = Frame(mCurrentFrame);
        mnLastKeyFrameId=mCurrentFrame.mnId;  //也可以通过mLastKeyFrame.mnId获得
        ...
        mpMap->mvpKeyFrameOrigins.push_back(pKFini);  //mvpKeyFrameOrigins什么作用？

        mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.mTcw);  //初始化的位姿为单位矩阵
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            fill(mvIniMatches.begin(),mvIniMatches.end(),-1); //给mvIniMatches中全填充-1。
                                                             // 之前没有给mvIniMatches分配空间
    ...
    else
    {
        ...
        //在调用SearchForInitalization时给mvIniMatches分配空间，空间大小为mInitialFrame.mvKeyUn.size(),和mvbPreMatched的大小一样
        int nmatches = matcher.SearchForInitialization(mInitialFrame,mCurrentFrame,mvbPrevMatched,mvIniMatches,100);
        //此时，mvbPrevMatched[i]=keypointCur. 即InitialFrame中的keypoint的id——i对应于CurrentFrame中的keypoint
        //mvIniMatches[i]=jCur，即InitialFrame中的keypoint的id——i对应于CurrenFrame中的keypoint的id——jCur.
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        MapPoint* pMP = new MapPoint(worldPos,pKFcur,mpMap); //注意这里用的是pkFCur,不是pKFini
    ...
    // 单目传感器无法恢复真实的深度，这里将pKFini中的mvpMapPoints在当pKFini相机坐标系下的深度取中指，作为medianDepth,归一化到1
    // 评估关键帧场景深度，q=2表示中值
    float medianDepth = pKFini->ComputeSceneMedianDepth(2);
    float invMedianDepth = 1.0f/medianDepth;

    if(medianDepth<0 || pKFcur->TrackedMapPoints(1)<100) //pKFcur中的mvpMapPoints被观测到的次数大于等于1的点的数量小于100的话
    {
    ...
    mvpLocalKeyFrames.push_back(pKFini);
    mvpLocalMapPoints=mpMap->GetAllMapPoints();
    //注意下边的ReferenceKF都是pKFcur不是pKFinit
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    vector<MapPoint*> vpMapPointMatches;  //vector的id表示F中keypoint的id， MapPoints*为pKF中的点
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        MapPoint* pMP = mLastFrame.mvpMapPoints[i];
        if(!pMP)   //如果lastframe中的keypoint[i]对应的MapPoint为空，则bCreateNew=true，表明需要创建MapPoint
            bCreateNew = true;
        else if(pMP->Observations()<1)  //如果lastframe中的keypoint[i]对应的MapPoint没有关键帧观测到，则也需要重新创建
        {
            bCreateNew = true;
        }
        ...
        if(vDepthIdx[j].first>mThDepth && nPoints>100)  //因为vDepthIdx是从小到大排序了的，
                                                      // 当vDepthIdx[j].first>mThDepth时，j后边的点也都大于mThDepth，就不要了
            break;
    }
}
...
bool Tracking::TrackLocalMap()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            // 通过PoseOptimization优化后，MapPoint仍能被观测到，就算是和keypoint匹配上了。
            ...
            else if(mSensor==System::STEREO) //TODO：为啥要判断是不是双目相机？
                mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
    ...
    // 步骤4：决定是否跟踪成功
    //如果最近才进行了relocalization，则追踪上的点应该更多一点。  mMaxFrames是帧率
...
{
    // 步骤1：如果用户在界面上选择只定位，那么将不插入关键帧
    // 由于插入关键帧过程中会生成MapPoint，因此用户选择只定位后地图上的点云和关键帧都不会再增加
    if(mbOnlyTracking)
        return false;
    ...
    // mMaxFrames等于图像输入的帧率
    // 如果关键帧数量比较少(nKFs<mMaxFrames)，则考虑插入关键帧
    ...
    if(nKFs<=2)
        nMinObs=2;
    int nRefMatches = mpReferenceKF->TrackedMapPoints(nMinObs); //返回ReferenceKF中的MapPoints被观测次数大于等于nMinObs的MapPoints的数量
    ...
    if((c1a||c1b||c1c)&&c2)
    {
        ...
        else
        {
            ...
            {
                // 队列里不能阻塞太多关键帧
                // tracking中插入关键帧不是直接插入，而且先插入到mlNewKeyFrames中，即，mpLocalMapper->InsertKeyFrame(pKF);
                // 然后localmapper再逐个pop出来进一步筛选后插入到mspKeyFrames
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // 这段代码和UpdateLastFrame中的那一部分代码功能相同，不同的是UpdateLastFrame生成的MapPoints是临时的用来跟踪的，
    // 不需要AddObservations等操作，而此处生成的是Keyframe中的MapPoints，是要长时间存储的，需要AddObservations等操作
...
void Tracking::SearchLocalPoints()
{
    ...
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        if(mCurrentFrame.isInFrustum(pMP,0.5)) //这些点pMP->mbTrackInView = true;将来要进行投影匹配
        {
        	// 观测到该点的帧数加1，该MapPoint在某些帧的视野范围内
            pMP->IncreaseVisible(); //仅仅是在视野范围内，还没有和该帧中的keypoint匹配上呢，所以不能用IncreaseFound
    ...
    if(nToMatch>0)
    {
        ...
        //将LocalMapPoints中被标记为pMP->mbTrackInView = true的点进行投影匹配，使CurrentFrame.mvpMapPoints变得更多
 ...
 *
 * 遍历当前帧的MapPoints，将观测到这些MapPoints的关键帧KFs 和与KFs相邻的关键帧取出，放入mvpLocalKeyFrames
 */
    ...
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        for(vector<KeyFrame*>::const_iterator itNeighKF=vNeighs.begin(), itEndNeighKF=vNeighs.end(); itNeighKF!=itEndNeighKF; itNeighKF++)
        {
            ...
            if(!pNeighKF->isBad())
            {
                ...
                if(pNeighKF->mnTrackReferenceForFrame!=mCurrentFrame.mnId)
                {
                    ...
                    break;  //pKF最佳共视的10帧只要有一帧被插入mvpLocalKeyframes就行了
                }
            }
        }
        ...
        for(set<KeyFrame*>::const_iterator sit=spChilds.begin(), send=spChilds.end(); sit!=send; sit++)
        {
            ...
            if(!pChildKF->isBad())
            {
                if(pChildKF->mnTrackReferenceForFrame!=mCurrentFrame.mnId)
                {
                    ...
                    break;  //pKF子关键帧只要有一帧被插入mvpLocalKeyframes就行了
                }
            }
        }
        ...
        if(pParent)
        {
            ...
            if(pParent->mnTrackReferenceForFrame!=mCurrentFrame.mnId)
            {
                ...
                break; //？？？只要pKF的父关键帧被插入mvpLocalKeyframes，直接跳出整个大循环，不检查其他关键帧了？？？
            }
        }
...
bool Tracking::Relocalization()
{
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            cv::Mat Tcw = pSolver->iterate(5,bNoMore,vbInliers,nInliers);  //迭代5次

            // If Ransac reachs max. iterations（300） discard keyframe
            if(bNoMore)
            {
                vbDiscarded[i]=true;
                nCandidates--;
