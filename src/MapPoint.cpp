    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap)
{
    ...
    mfMaxDistance = dist*levelScaleFactor; //此处的dist的scale是pFrame提取该Map的图像特征所处的金字塔层数的scale， 乘上levelScaleFactor，将dist的scale变成图像金字塔底层的scale
    mfMinDistance = mfMaxDistance/pFrame->mvScaleFactors[nLevels-1];  //此处的distance对应与图像金字塔最上层的scale
...
void MapPoint::Replace(MapPoint* pMP)
{
    ...
    map<KeyFrame*,size_t> obs;// 这一段和SetBadFlag函数相同
    {
        ...
        nvisible = mnVisible;
        nfound = mnFound;
        mpReplaced = pMP;   //this MapPoint 被替换为pMp，以mpReplaced来记录替换点。
    }

    // 所有能观测到该MapPoint的keyframe都要替换。obs记录着能观察到this MapPoint的keyframe
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        // Replace measurement in keyframe
        KeyFrame* pKF = mit->first;

        if(!pMP->IsInKeyFrame(pKF)) //如果替换点pMp中的mObservations中没有 "观察到this MapPoint的keyframe(pKF)"，那么就应该将pKF加进去。
        {
            pKF->ReplaceMapPointMatch(mit->second, pMP);// keyframe(pKF)所观察到的this MapPoint被替换为pMp
            pMP->AddObservation(pKF,mit->second);// 将pKF加入到pMp的mObservations中
        }
        else //如果替换点pMp中的mObservations中有 "观察到this MapPoint的keyframe(pKF)"，说明this MapPoint和pMP冲突了，应该将this MapPoint从pKF中删除
        {
    ...
    pMP->IncreaseFound(nfound);      //pMP的nfound，nvisible应该与this MapPoint相同，加上。
    pMP->IncreaseVisible(nvisible);
    pMP->ComputeDistinctiveDescriptors();  //重新计算pMP的描述子。
