 ...
 * 2. Edge:
 *     - g2o::EdgeSE3ProjectXYZ() --单目  EdgeStereoSE3ProjectXYZ()--双目，RGBD  都是二元边
 *         + Vertex：待优化当前帧的Tcw
                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        vSE3->setEstimate(Converter::toSE3Quat(pKF->GetPose())); //将SE3转为四元数和t的形式
        vSE3->setId(pKF->mnId);
        vSE3->setFixed(pKF->mnId==0); //如果pKF是第一帧，则将这个pKF设为Fixed
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        vPoint->setEstimate(Converter::toVector3d(pMP->GetWorldPos())); //将Mat转成eigen
        const int id = pMP->mnId+maxKFid+1; //id要比pKF的最大id大，这样才能避免id冲突
        vPoint->setId(id);
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(pKF->mvuRight[mit->second]<0)
            {
                ...
                //setVertex的顺序得和边的定义中的顺序一样，先是点，再是位姿
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // Set MapPoint vertices
    const int N = pFrame->N;  //当前帧提取的特征点数量

    // for Monocular
    vector<g2o::EdgeSE3ProjectXYZOnlyPose*> vpEdgesMono;
    vector<size_t> vnIndexEdgeMono;
    vpEdgesMono.reserve(N);   //每个MapPoint都对应一条一元边，该边的顶点的pFrame的位姿（因为只优化位姿）
    vnIndexEdgeMono.reserve(N);  //存放MapPoint对应的keypoint在pFrame中的id
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            if(pFrame->mvuRight[i]<0)
            {
                ...
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));  //添加pFrame的位姿
                e->setMeasurement(obs);
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            {
                e->computeError(); // NOTE g2o只会计算active edge(即level=0）的边的误差。
                                   //每次优化完，当前edge可能在上次优化的时候是level=1,所以当前边没有计算误差，也就没有chi2,
                                   //所以需要我们手动计算。
            }
            ...
            else    //这条边对应的mapPoint可能又会被设为inlier
            {
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    pKF->mnBALocalForKF = pKF->mnId;  //标记该关键帧是以pKF为主的LocalKeyFrame
    ...
    for(int i=0, iend=vNeighKFs.size(); i<iend; i++)
    {
        ...
        pKFi->mnBALocalForKF = pKF->mnId; //标记该关键帧是以pKF为主的LocalKeyFrame
        if(!pKFi->isBad())
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                    if(pMP->mnBALocalForKF!=pKF->mnId)
                    {
                        ...
                        pMP->mnBALocalForKF=pKF->mnId;// 防止重复添加，标记该点是以pKF为主的LocalMapPoint
                    }
            }
        }
    }
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            {                
                pKFi->mnBAFixedForKF=pKF->mnId;// 防止重复添加，标记这些关键帧是能观测到以pKF为主的LocalMapPoints的关键帧且这些关键帧不是以pKF为主的LocalKeyFrame。这些关键帧在优化的时候是fixed的
                if(!pKFi->isBad())
    ...
    //边的数量。 每个KefFrame和每个MapPoint都有一条边
    const int nExpectedSize = (lLocalKeyFrames.size()+lFixedCameras.size())*lLocalMapPoints.size();

    vector<g2o::EdgeSE3ProjectXYZ*> vpEdgesMono;
    vpEdgesMono.reserve(nExpectedSize);  //存储边

    vector<KeyFrame*> vpEdgeKFMono;
    vpEdgeKFMono.reserve(nExpectedSize);  //存储与vpEdgesMono中每一个二元边的顶点（KerFrame）

    vector<MapPoint*> vpMapPointEdgeMono;
    vpMapPointEdgeMono.reserve(nExpectedSize); //存储与vpEdgesMono中每一个二元边的另一个顶点（MapPoint）
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    for(map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend=LoopConnections.end(); mit!=mend; mit++)
    {
        ...
        for(set<KeyFrame*>::const_iterator sit=spConnections.begin(), send=spConnections.end(); sit!=send; sit++)
        {
            ...
            //对于与pCurKF或LoopKF无关的顶点，只有当共视点足够多（大于等于100）时，才加入该条边，否则continue跳过
            //只要与pCurKF或LoopKF有关，就需要加入该边
            ...
            e->setMeasurement(Sji); //？？？ 这样的话计算的来的误差为0
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        g2o::EdgeSim3ProjectXYZ* e12 = new g2o::EdgeSim3ProjectXYZ();  //MapPoint2 通过Sim12投影到KF1上的点
        ...
        g2o::EdgeInverseSim3ProjectXYZ* e21 = new g2o::EdgeInverseSim3ProjectXYZ(); //将MapPoint1 通过Sim12投影到KF2上的点。
                                                                                   //注意这里的边是g2o::Edge“Inverse”Sim3ProjectXYZ()
